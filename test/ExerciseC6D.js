...
contract("ExerciseC6D", async accounts => {
  ...
  before("setup contract", async () => {
    ...
    events.watch((error, result) => {
      if (result.event === "OracleRequest") {
        index = result.args.index.toNumber();
        console.log(index);
        console.log(
  ...
  it("can request flight status", async () => {
    ...
    await config.exerciseC6D.fetchFlightStatus.call(flight, timestamp);
    ...
    for (let a = 1; a < TEST_ORACLES_COUNT; a++) {
      ...
      for (let idx = 0; idx < 3; idx++) {
        // if (oracleIndexes[idx].eq(index)) {
        try {
          console.log(oracleIndexes[idx], flight, timestamp, 10, {
            from: accounts[a],
          });
